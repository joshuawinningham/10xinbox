          ...
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  const data = await res.json();
                  console.log('Debug response time data:', JSON.stringify(data, null, 2));
            ...
            <button
              onClick={async () => {
                ...
                setResponseError(null);
                try {
                  console.log('Manually refreshing response time...');
                  const res = await fetch(`${import.meta.env.VITE_API_URL}/api/gmail/response-time`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ user_id: user.id }),
                  ...
                  if (!res.ok) throw new Error('Failed to fetch response time');
                  const data = await res.json();
                  console.log('Manual refresh response time data:', data);
                  setResponseTime(data.average_response_time);
                  setResponseCount(data.count);
                } catch (err) {
